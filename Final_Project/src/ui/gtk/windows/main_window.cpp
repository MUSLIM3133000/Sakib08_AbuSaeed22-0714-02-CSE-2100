/**
 * @file ui/gtk/windows/main_window.cpp
 * @brief Main window implementation (C++17)
 *
 * C improvements:
 *  - g_malloc0(sizeof(EventViewerContext)) + manual g_free
 *    → Context allocated in main(), passed in via userData (DIP)
 *  - ctx->current_log_handle (EVT_HANDLE, manual EvtClose)
 *    → removed — log lifetime owned by ILogRepository
 *  - ctx->current_log_name = NULL + strdup
 *    → ctx->currentLogName is std::string — no manual memory
 *  - NULL → nullptr throughout
 *  - PopulateTree/PopulateTables free-function calls
 *    → EventModels::populateTree / populateTables (namespaced class methods)
 *  - EventStatistics_Calculate() called directly here in C
 *    → statistics fetched via ctx->logRepository and passed to populateTables (DIP)
 */
...
void MainWindow::activate(GtkApplication* app, gpointer userData) {
    // DIP: context is pre-built in main() with injected repository/exporter
    ...
    // Populate sidebar tree (pure GTK — no data dependency)
    EventModels::populateTree(ctx);

    // DIP: fetch stats via ILogRepository (not direct Windows API call)
    ...
    EventModels::populateTables(ctx, stats1h, stats24h, stats7d);
...
// ─────────────────────────────────────────────────────────────────────────────
// Private helpers (same structure as C version, cleaned up)
// ─────────────────────────────────────────────────────────────────────────────
static GtkWidget* createContentArea(EventViewerContext* ctx) {
    ...
    // Header with title and refresh timestamp
