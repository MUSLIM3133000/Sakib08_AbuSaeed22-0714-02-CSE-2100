/**
 * @file ui/models/event_models.cpp
 * @brief EventModels implementation (C++17)
 *
 * C improvements:
 *  - EventModels_PopulateTables() called EventStatistics_Calculate() directly
 *    → now receives pre-computed stats (DIP, testable, no Windows coupling)
 *  - EventModels_LoadLogEvents() called EventLog_Windows_Open/Read directly
 *    → now calls ctx->logRepository->read() (ILogRepository interface)
 *  - Raw EventRecord* + int count  →  const std::vector<EventRecord>&
 *  - strcmp(log_name, "Application") chain  →  std::string comparison
 *  - manual strdup(log_name)  →  ctx->currentLogName = logName (std::string)
 *  - EvtClose(ctx->current_log_handle) manually  →  repository manages lifetime
 */
...
// ─────────────────────────────────────────────────────────────────────────────
// populateTree — identical structure to C, now inside a class
// ─────────────────────────────────────────────────────────────────────────────
void EventModels::populateTree(EventViewerContext* ctx) {
...
// ─────────────────────────────────────────────────────────────────────────────
// populateTables — stats injected (DIP), no direct Windows API call
// ─────────────────────────────────────────────────────────────────────────────
void EventModels::populateTables(EventViewerContext*    ctx,
                                  ...
                                  const EventStatistics& stats7d)
{
    ...
    // Log summary table (static data — same as C version)
...
// ─────────────────────────────────────────────────────────────────────────────
// populateEventDetails — vector replaces raw pointer + count
// ─────────────────────────────────────────────────────────────────────────────
void EventModels::populateEventDetails(EventViewerContext*             ctx,
...
// ─────────────────────────────────────────────────────────────────────────────
// loadLogEvents — calls ILogRepository, no direct Windows API
// ─────────────────────────────────────────────────────────────────────────────
void EventModels::loadLogEvents(EventViewerContext* ctx,
                                 const std::string&  logName)
{
    gtk_list_store_clear(ctx->eventDetailsStore);

    // Convert log name to wide string for ILogRepository
    const wchar_t* wname = nullptr;
    std::wstring   wnameStorage;
    ...
    // DIP: call through ILogRepository interface — no Windows API here
    ...
    if (!events.empty()) {
        ...
        ctx->currentLogName = logName;   // std::string — no strdup/free
    } else {
